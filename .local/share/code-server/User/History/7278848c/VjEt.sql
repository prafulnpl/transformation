    ...
    {%- set json_keys_query %}
        ...
        FROM (
            ...
            FROM {{ relation }},
                 UNNEST(
                     CASE 
                         WHEN json_parse({{ json_column }}) IS NOT NULL 
                         THEN map_keys(CAST(json_parse({{ json_column }}) AS MAP(VARCHAR, JSON)))
                         ELSE ARRAY[] 
                     END
                 ) AS t(key)
        )
    ...
    SELECT
        *,
        {%- for idx in range(keys | length) %}
            CASE
                -- Extract both simple and nested keys using double quotes for safety
