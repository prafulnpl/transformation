{% macro flatten_json_trino(table, json_column, alias_prefix="") %}
    ...
    -- Step 3: Sanitize keys to create SQL-friendly column names
    {%- set sanitized_keys = [] %}
    {%- for key in keys %}
        {%- set sanitized_key = key | replace(' ', '_') | replace('-', '_') | replace(':', '_') | replace('.', '_') %}
        {%- if sanitized_key | regex_search('^\d') %}
            {%- set sanitized_key = 'key_' + sanitized_key %}  {# Prefix numeric keys with 'key_' #}
        {%- endif %}
