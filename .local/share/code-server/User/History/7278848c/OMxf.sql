{% macro flatten_json_trino(table, json_column, alias_prefix="") %}
    ...
    -- Step 4: Generate SQL to flatten the JSON, handling arrays explicitly
    SELECT
        *,
        {%- for idx in range(keys | length) %}
            CASE
                -- Check if the JSON value is an array or object using json_format() with LIKE
                WHEN json_format(json_extract({{ json_column }}, '$.{{ escaped_keys[idx] }}')) LIKE '[%' THEN
                    json_format(json_extract({{ json_column }}, '$.{{ escaped_keys[idx] }}'))
                ELSE
