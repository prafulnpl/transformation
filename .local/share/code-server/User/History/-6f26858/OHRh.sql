...
SELECT
    st_studentid,
    st_fullname,  -- Full Name will come from 'silver_studentmain'
    wf.name AS form_name,  -- Overriding with 'name' from silver_websiteform
    st_classname,
    st_facultyname,
    st_departmentname,
    st_ayear,
    ...
    wf.phone_number AS form_phone_number,  -- Ensure only one column for phone number
...
FULL OUTER JOIN
    ...
    ON wf.phone_number = rs.st_parmanentphone  -- Match on phone number
WHERE
    rs.rn = 1
